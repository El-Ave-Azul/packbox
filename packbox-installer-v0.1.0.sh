...
install_go() {
    ...
    if command -v go &>/dev/null; then
        ...
        maj=$(echo "$v" | cut -d. -f1)
        min=$(echo "$v" | cut -d. -f2)
        # EN: Accept 1.x with x >= 22, or any 2.x+.
        # ES: Acepta 1.x con x >= 22, o cualquier 2.x+.
        if [[ "$maj" -gt 1 ]] || { [[ "$maj" -eq 1 ]] && [[ "$min" -ge 22 ]]; }; then
            ok "Go $v"
            return 0
        fi
    fi
...
func AnalyzeDependencies(p string) ([]string, error) {
	...
	for s.Scan() {
		l := s.Text()
		// EN: Keep only resolved "=>" lines; skip "not found".
		// ES: Conserva solo líneas "=>" resueltas; omite "not found".
		if strings.Contains(l, "=>") && !strings.Contains(l, "not found") {
...
func (s *Sandbox) addGUISupport(args *[]string) {
	...
	if rd := os.Getenv("XDG_RUNTIME_DIR"); rd != "" {
		*args = append(*args, "--setenv", "XDG_RUNTIME_DIR", rd)
		...
		// EN: sock — avoid shadowing the *Sandbox receiver.
		// ES: sock — evita ocultar el receiver *Sandbox.
		for _, sock := range []string{"pulse", "pipewire-0", "pipewire-0-manager"} {
			p := filepath.Join(rd, sock)
...
func main() {
	...
	var missing []string
	// EN: resolved — count of libs found on the host.
	// ES: resolved — nº de libs encontradas en el host.
	resolved := 0
...
# ═══════════════════════════════════════════════════════════════
main() {
    # EN: Bootstrap i18n. Write the per-language files first so
    #     select_language() can persist the choice into the lang
    #     dir, then load the chosen file.
    # ES: Arranca i18n. Escribe primero los archivos de idioma
    #     para que select_language() pueda persistir la elección
    #     en el dir de idiomas, y luego carga el archivo elegido.
    install_lang_files
    select_language
    load_lang
