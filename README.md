...
### 1. Inglés — `README.md` (Archivo principal)

# 📦 Packbox v0.1.0 Alpha

*9 languages / 9 idiomas: EN · [ES](README-es.md) · [FR](README-fr.md) · [DE](README-de.md) · [IT](README-it.md) · [zh-CN](README-zh-CN.md) · [zh-TW](README-zh-TW.md) · [JA](README-ja.md) · [KO](README-ko.md)*

Next-generation application packaging system, inspired by Flatpak but built on a fundamentally different reuse model.

## 🌟 What Makes Packbox Different

| Feature | Flatpak (current) | Packbox (proposed) |
| :--- | :--- | :--- |
| **Unit of reuse** | Full runtime (~1 GB) | Atomic cell (~5–50 MB) |
| **Deduplication** | File-level (OSTree) | Chunk-level (CAS) |
| **Library sharing** | Within same runtime | Cross-app sharing |
| **Host library usage** | None (full sandbox) | Selective (ABI compatible) |
| **Updates** | OSTree object delta | Chunk delta + reordering |
| **Per-app overhead** | ~100% if runtime differs | ~5–15% (differences only) |

**Core idea:** Instead of shipping a monolithic runtime per app, Packbox stores every file as content-addressed *chunks*. Two apps sharing 90% of their libraries only store the 10% that differs. Library reuse across apps is automatic.
...
1. Detects your distro (Debian/Ubuntu families, Fedora, Arch, openSUSE).
...
6. Installs language files in `~/.config/packbox/lang/`.
    

After installation, reload your environment and verify:
...
```
~/.packbox/                     Installation (binaries + Go source)
├── bin/                        11 Go binaries
...
All commands are installed in `~/.packbox/bin/` and symlinked in `~/.local/bin/`. They accept `--help` where indicated.

- `packbox-pack`: Packages a directory → CAS chunks + manifest.
    
- `packbox-install`: Installs app from manifest (hardlinks from CAS).
    
- `packbox-run`: Runs app in bwrap sandbox (with DNS/GUI fix).
    
- `packbox-list`: Lists installed apps.
    
- `packbox-remove`: Uninstalls app and releases CAS references.
    
- `packbox-gc`: Garbage collects orphaned chunks.
    
- `packbox-verify`: Verifies library compatibility (ldd).
    
- `packbox-export`: Exports app to .pbox (zstd/xz/gzip).
    
- `packbox-import`: Imports .pbox with anti path-traversal validation.
    
- `packbox-module`: Manages shared library modules.
    
- `packbox-diagnose`: Diagnoses environment (dirs, tools, DNS, kernel).
    

## 🏗️ Manifest Format (`manifest.json`)
...
- **CAS (Content-Addressed Store):** Hash uses BLAKE3 (32 bytes = 64 hex characters). The `isValidHash()` function blocks path traversal attacks (e.g. `../../etc/passwd`) if hex characters do not match.
    
- **Sandbox (bwrap):** Mounts the application tree as read-only at `/app` and applies read-only bind-mounts to system directories (`/usr`, `/etc`, `/lib`, `/bin`, etc.).
    
- **XAUTHORITY Handling:** To secure graphical authorization, Packbox copies the host file to `/tmp/packbox-xauth-<pid>` (mode 0600) so the app reads its own X auth rather than the system host's auth.
    

## 📄 License / Contributing

Packbox v0.1.0 Alpha is a work in progress. Contributions are welcome, especially: translations, additional sandbox profiles, and CAS chunking strategies.
